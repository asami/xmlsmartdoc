 ...
 */
public class Bibliography extends TitledBlock {
    public Bibliography(Element element) {
	super(element);
	...
	for (int i = 0;i < size;i++) {
		...
		Bibitem item = Bibitem.getBibitem(child);
		if (item != null) {
		    addContent(item);
		}
		break;
    ...
    public Bibliography(Element element, BibliographyDatabase bibDB) {
	super(element);
	NodeList nodes = element.getChildNodes();
	int size = nodes.getLength();
	for (int i = 0;i < size;i++) {
	    ...
	    case Node.ELEMENT_NODE:
		Element child = (Element)node;
		addContent(Bibitem.getBibitem(child, bibDB));
		break;
	    case Node.TEXT_NODE:
	    case Node.ENTITY_REFERENCE_NODE:
	    case Node.COMMENT_NODE:
