...
/**
 * A data manipulation object for a table 'accounts'.
 *
 * @version account.rng 1.0 (Tue Sep 09 05:58:16 GMT+09:00 2003)
 * @author  Relaxer 1.0rc3b (http://www.relaxer.org)
 */
public class AccountsTable implements IAccountsTableView {
    ...
    /**
     * Creates a <code>AccountsTable</code>.
     *
     * @param url
     * @param tableName
     * @exception SQLException
     */
    public AccountsTable(String url, String tableName) throws SQLException {
     ...
     * @exception SQLException
     */
    public AccountsTable(String url, String tableName, String username, String password) throws SQLException {
     ...
     * @exception SQLException
     */
    public AccountsTable(String url, String tableName, java.util.Properties properties) throws SQLException {
        conn_ = DriverManager.getConnection(url, properties);
        tableName_ = tableName;
    }

    /**
     * Creates a <code>AccountsTable</code> by a Connection.
     *
     * @param connection
     * @param tableName
     */
    public AccountsTable(Connection connection, String tableName) {
        conn_ = connection;
        tableName_ = tableName;
    }
    ...
    public void createTable() throws SQLException {
        ...
        try {
            st.execute(
                "CREATE TABLE " + getQuoteId() + tableName_ + getQuoteId() + " (" + getQuoteId() + "account" + getQuoteId() + " VARCHAR(512) NOT NULL)"
            );
        } finally {
            st.close();
        }
    ...
    public void insert(Element element) throws SQLException {
        if (insertStatement_ == null) {
            insertStatement_ = conn_.prepareStatement(
                "INSERT INTO " + getQuoteId() + tableName_ + getQuoteId() + " (" + getQuoteId() + "account" + getQuoteId() + ")" + " VALUES (?)"
            );
        }
        PreparedStatement st = insertStatement_;
        st.setString(1, USQLNS.getHedgePropertyAsXMLString(element, "<http://www.relaxer.org/xmlns/framework/account>account"));
        st.execute();
    }
     ...
     * @exception SQLException
     */
    public void insert(RAccounts object) throws SQLException {
        try {
     ...
     * @exception SQLException
     */
    public void insert(RAccounts[] objects) throws SQLException {
        boolean autoCommit = conn_.getAutoCommit();
        if (autoCommit) {
            conn_.setAutoCommit(false);
        }
    ...
    public void insert(Collection collection) throws SQLException {
        ...
        while (iter.hasNext()) {
            ...
            } else if (object instanceof RAccounts) {
                insert((RAccounts)object);
            } else {
    ...
    public static void main(String[] args) throws Exception {
        ...
        AccountsTable table = new AccountsTable(url, tableName, username, password);
